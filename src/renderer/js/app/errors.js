...
(function () {
  ...
  var CATALOG = [
    ...
    [/corrupt.*decryption failed|vault key may have changed|re-enter the credential/i, "err.vault.corrupt",
      "A saved credential can't be decrypted (the vault key may have changed). Re-enter the credential in its settings page — nothing was sent with an empty password."],
    [/no sha256|hash-less update|sha256 mismatch/i, "err.update.hash",
      "Update refused: the release has no valid integrity hash. This protects you from tampered updates — contact the release publisher.", "open-engine"],
    [/invalid version.*dotted numerics|release-store/i, "err.update.version",
      "Update refused: invalid release version. The manifest may be tampered with.", "open-engine"],
    [/manifestUrl.*not allowed|resolves to a blocked|file:\/\/.*not allowed/i, "err.update.manifest-url",
      "Custom update-manifest URL rejected: only public http(s) hosts are allowed (no local files or private IPs)."],
    [/placeholder API token|set a real token/i, "err.token.placeholder",
      "The API server refused to start with a placeholder token. Set a real AGENT_BROWSER_API_TOKEN.", "open-engine"],
    [/config\.json is corrupt|refusing to run on factory defaults/i, "err.config.corrupt",
      "The configuration file is corrupt — the app refused to start on empty defaults to protect your data. Restore the backup shown in the error, or remove the file to start fresh (data loss!)."],
    [/injection probe blocked|undecidable probe|zero injection/i, "err.probe.blocked",
      "Launch blocked: the fingerprint injection could not be verified (the probe was undecidable or dead). The profile did NOT launch with zero injection. Retry, or set blockOnInjectionProbe=false to launch unverified.", "open-engine"],
    [/queued-cookie|cookie.*failed/i, "err.cookies.failed",
      "Some queued cookies could not be applied — the profile launched without part of its saved session. Check the launch details."],
    [/exited early.*before announcing/i, "err.engine.exited",
      "The browser exited immediately on launch (before announcing its debugging port). The binary or profile may be broken — try relaunching, or check the engine install.", "open-engine"],
  ];
