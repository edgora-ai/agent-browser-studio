...
export const TASK_TEMPLATES: TaskTemplate[] = [
  ...
  },
  {
    ...
    title: "优惠码可用性巡检",
    category: "ecommerce",
    description: "批量检查优惠码在目标站点购物车中是否可用/生效,结果写入 coupons 表。",
    riskLevel: "medium",
    requiredInputs: [
      { key: "checkoutUrl", label: "购物车/结算页 URL", description: "添加商品后能看到优惠码输入框的页面。", required: true },
      { key: "coupons", label: "优惠码列表", description: "一行一个优惠码。", required: true, example: "SAVE10" },
      { key: "profile", label: "登录 Profile", description: "带登录态的 Browser profile。", required: true },
    ],
    tools: ["browser_navigate", "browser_wait_for_load", "browser_input", "browser_snapshot", "browser_evaluate", "db_exec"],
    successCriteria: ["coupons 表存在", "每个优惠码都有 status(valid/invalid/error)", "记录折扣金额(若能解析)", "异常页面在总结中列出"],
    examplePrompt: "用 coupon-check 模板批量检查这些优惠码是否可用: SAVE10, WELCOME5 (结算页 https://shop.example/cart)",
    prompt: "对每个优惠码:打开购物车页,填入优惠码,读取提示/折扣金额,把结果写入 coupons 表。",
    outputTable: { name: "coupons", columns: ["code", "status", "discount", "currency", "checked_at"] },
    steps: [
      "db_exec: CREATE TABLE IF NOT EXISTS coupons (id INTEGER PRIMARY KEY, code TEXT, status TEXT, discount REAL, currency TEXT, checked_at TEXT)",
      "browser_navigate(checkoutUrl) → browser_wait_for_load",
      "对每个优惠码:browser_input 填入 → browser_snapshot 读取提示/折扣 → 判断 valid/invalid",
      "db_exec: 参数化 INSERT 结果",
      "总结有效/无效/失败数量",
    ],
  },
  {
    ...
    title: "库存/到货监控",
    category: "ecommerce",
    description: "定时监控商品库存状态,补货/缺货变化写入 stock_alerts 表。",
    riskLevel: "low",
    requiredInputs: [
      { key: "urls", label: "商品 URL 列表", description: "一行一个。", required: true },
      { key: "threshold", label: "告警阈值", description: "库存低于该值即记录,默认 0(仅记录有货/无货)。", required: false, example: "0" },
    ],
    tools: ["browser_navigate", "browser_wait_for_load", "browser_evaluate", "db_exec"],
    successCriteria: ["stock_alerts 表存在", "每个商品都写入 in_stock/out_of_stock 与数量", "与上次快照不同才插入(避免重复)", "总结有货商品"],
    examplePrompt: "用 stock-alert 模板监控这些商品库存并写入 stock_alerts: https://shop.example/products/1",
    prompt: "监控商品库存状态,把每次快照写入 stock_alerts:url/in_stock/quantity/checked_at,仅记录变化。",
    outputTable: { name: "stock_alerts", columns: ["url", "in_stock", "quantity", "checked_at"] },
    steps: [
      "db_exec: CREATE TABLE IF NOT EXISTS stock_alerts (id INTEGER PRIMARY KEY, url TEXT, in_stock INTEGER, quantity INTEGER, checked_at TEXT)",
      "对每个 URL: browser_navigate → browser_wait_for_load → browser_evaluate 提取库存状态",
      "查询上次快照,若状态未变化则跳过写入",
      "db_exec: 参数化 INSERT 有变化记录",
      "总结有货/缺货商品",
    ],
  },
  {
    ...
    title: "私信待办巡检",
    category: "social",
    description: "巡检私信/消息列表,标记需要回复的消息,写入 dm_log 表。",
    riskLevel: "medium",
    requiredInputs: [
      { key: "messagesUrl", label: "消息列表 URL", description: "私信/消息中心页面。", required: true },
      { key: "limit", label: "条数", description: "默认前 20 条。", required: false, example: "20" },
    ],
    tools: ["browser_navigate", "browser_wait_for_load", "browser_evaluate", "browser_snapshot", "db_exec"],
    successCriteria: ["dm_log 表存在", "写入 sender/preview/url/needs_reply", "按未读或关键词标记 needs_reply", "不主动发送消息(只巡检)"],
    examplePrompt: "用 dm-response 模板巡检消息列表前 20 条并写入 dm_log: https://social.example/messages",
    prompt: "巡检消息列表,把每条消息写入 dm_log:sender/preview/url/needs_reply,needs_reply 按未读或关键词判断。",
    outputTable: { name: "dm_log", columns: ["sender", "preview", "url", "needs_reply", "checked_at"] },
    steps: [
      "db_exec: CREATE TABLE IF NOT EXISTS dm_log (id INTEGER PRIMARY KEY, sender TEXT, preview TEXT, url TEXT, needs_reply INTEGER, checked_at TEXT)",
      "browser_navigate(messagesUrl) → browser_wait_for_load → browser_evaluate 提取消息列表",
      "对每条判断 needs_reply(未读/含关键词)",
      "db_exec: 参数化 INSERT,URL 去重",
      "总结待回复数量",
    ],
  },
  {
    ...
    title: "广告系列数据汇总",
    category: "ads",
    description: "拉取广告后台系列/广告组的展示、点击、花费、转化,写入 campaign_metrics。",
    riskLevel: "medium",
    requiredInputs: [
      { key: "dashboardUrl", label: "广告后台数据页 URL", description: "包含数据表格的后台页面。", required: true },
      { key: "accounts", label: "账号/Profile", description: "要采集的账号范围。", required: true },
    ],
    tools: ["browser_navigate", "browser_wait_for_load", "browser_evaluate", "db_exec"],
    successCriteria: ["campaign_metrics 表存在", "写入 campaign/impressions/clicks/spend/conversions/date", "按日期+系列去重", "总结各系列花费"],
    examplePrompt: "用 campaign-report 模板拉取广告系列数据并写入 campaign_metrics: https://ads.example/dashboard",
    prompt: "拉取广告系列数据表格,把每行写入 campaign_metrics:campaign/impressions/clicks/spend/conversions/date。",
    outputTable: { name: "campaign_metrics", columns: ["campaign", "impressions", "clicks", "spend", "conversions", "date"] },
    steps: [
      "db_exec: CREATE TABLE IF NOT EXISTS campaign_metrics (id INTEGER PRIMARY KEY, campaign TEXT, impressions INTEGER, clicks INTEGER, spend REAL, conversions INTEGER, date TEXT)",
      "browser_navigate → browser_wait_for_load → browser_evaluate 提取表格行",
      "按 campaign+date 去重",
      "db_exec: 参数化 INSERT",
      "总结各系列花费与 CTR",
    ],
  ...
  {
    id: "proxy-rotation-check",
    title: "代理出口巡检",
    category: "ops",
    description: "检查各代理当前出口 IP 与地区,记录变化与可用性到 proxy_health_check 表。",
    riskLevel: "low",
    requiredInputs: [
      { key: "proxies", label: "代理名称", description: "要巡检的代理名,逗号分隔。", required: true, example: "hk-01, us-02" },
    ],
    tools: ["http_request", "db_exec"],
    successCriteria: ["proxy_health_check 表存在", "每个代理写入 exit_ip/country/status/at", "与上次不同才插入(记录变化)", "超时/失败代理在总结中列出"],
    examplePrompt: "用 proxy-rotation-check 模板巡检所有代理出口 IP 并写入 proxy_health_check",
    prompt: "通过代理发起请求查询出口 IP 与地区,把结果写入 proxy_health_check:proxy/exit_ip/country/status/at,仅记录变化。",
    outputTable: { name: "proxy_health_check", columns: ["proxy", "exit_ip", "country", "status", "at"] },
    steps: [
      "db_exec: CREATE TABLE IF NOT EXISTS proxy_health_check (id INTEGER PRIMARY KEY, proxy TEXT, exit_ip TEXT, country TEXT, status TEXT, at TEXT)",
      "对每个代理:http_request 经代理访问 IP 查询接口(如 https://api.ipify.org?format=json + 地区接口)",
      "比较上次记录,变化才插入",
      "db_exec: 参数化 INSERT",
      "总结可用代理与故障代理",
    ],
