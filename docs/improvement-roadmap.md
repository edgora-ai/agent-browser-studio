...
**诚实边界**：preload JS 注入证据强度低于 Chromium 原生 patch（`fingerprintParity:false` 如实上报）；Worker/DedicatedWorker 内的指纹面仍无法经 preload 覆盖（OpenBSD 侧同理属引擎所致，非注入缺口）；Firefox 真实指纹一致性已在真机 + 两世界对照打通（81.3，判据全成立），外部指纹平台比对（Slice 72 方法）已落地（81.4/81.5）并归因为站点口径与引擎边界；Firefox UA 为引擎级字段（`navigator.userAgent` 两世界一致，不 rewrite）；无交互/低活跃窗的 Firefox rAF 呈停滞-追赶模式，`raf_timing` 测量反映环境合成器特征而非 persona（81.5 定案）；WebRTC `proxy_only` 以无 STUN 直连候选换取真 IP 零暴露（未配 TURN 时媒体直连退化）；DNS 观测残留源自代理路径（socks5h/网关为根解，待办）。
