...
**诚实边界**：preload JS 注入证据强度低于 Chromium 原生 patch（`fingerprintParity:false` 如实上报）；Worker/DedicatedWorker 内的指纹面仍无法经 preload 覆盖（OpenBSD 侧同理属引擎所致，非注入缺口）；Firefox 真实指纹一致性已在真机 + 两世界对照打通（81.3，判据全成立），外部指纹平台比对（Slice 72 方法）仍待有环境时进行；Firefox UA 为引擎级字段（`navigator.userAgent` 两世界一致，不 rewrite）。
