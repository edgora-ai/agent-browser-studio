...
const child = spawn(findFirefoxBinary(), buildFirefoxLaunchArgs({ profileDir, remotePort, headless: false, platform: "windows" }), { stdio: ["ignore", "pipe", "inherit"], env });
child.on("exit", (code, signal) => console.error("FF EXIT code=" + code + " signal=" + signal));
child.stderr.on("data", d => process.stderr.write("FFERR " + String(d).slice(0, 700) + "\n"));
...
await bidiNavigate(conn, "https://ping0.cc/", ctx, 60000);
await wf(15000);
